...
# Options: see kk_runner_show_help
...
# Load test framework
source "$(dirname "$SCRIPT_DIR")/kk-test.sh"

# Parse command line arguments
kk_runner_parse_args "$@"

# Show test execution info
kk_test_section "Starting Comprehensive Test Suite"

# Find test files
test_files=()
while IFS= read -r file; do
    test_files+=("$file")
done < <(kk_runner_find_tests "$SCRIPT_DIR")
