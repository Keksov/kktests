...
kk_test_init "AdvancedFileSystemAssertions" "$(dirname "$0")"

TMPDIR=$(kk_fixture_tmpdir)

# Test kk_assert_file_not_exists with non-existent file
kk_test_start "kk_assert_file_not_exists with non-existent file"
if kk_assert_file_not_exists "/nonexistent/path/file.txt" "Non-existent file"; then
...
# Test kk_assert_dir_not_exists with non-existent directory
kk_test_start "kk_assert_dir_not_exists with non-existent directory"
if kk_assert_dir_not_exists "/nonexistent/directory" "Non-existent directory"; then
    kk_test_pass "Assertion passed"
else
    kk_test_fail "Assertion failed"
fi
...
# Test kk_assert_file_readable with readable file
kk_test_start "kk_assert_file_readable with readable file"
readable_file=$(kk_fixture_tmpfile "readable")
if kk_assert_file_readable "$readable_file" "Readable file"; then
    kk_test_pass "Assertion passed"
else
    kk_test_fail "Assertion failed"
fi

# Test kk_assert_file_readable fails with unreadable file
kk_test_start "kk_assert_file_readable fails with unreadable file"
...
# Test kk_assert_file_writable with writable file
kk_test_start "kk_assert_file_writable with writable file"
writable_file=$(kk_fixture_tmpfile "writable")
if kk_assert_file_writable "$writable_file" "Writable file"; then
    kk_test_pass "Assertion passed"
else
    kk_test_fail "Assertion failed"
fi

# Test kk_assert_file_writable fails with read-only file
...
kk_test_start "File assertions with spaces in paths"
space_dir=$(kk_fixture_tmpdir_create "dir with spaces")
space_file="$space_dir/file with spaces.txt"
touch "$space_file"
if kk_assert_file_exists "$space_file" "File with spaces in path"; then
    kk_test_pass "Assertions work with space-containing paths"
else
    kk_test_fail "Assertions failed with space-containing paths"
fi

# Test with symbolic links
kk_test_start "File assertions with symbolic links"
real_file=$(kk_fixture_tmpfile "realfile")
...
kk_test_start "File assertions with hidden files"
hidden_file="$TMPDIR/.hiddenfile"
touch "$hidden_file"
if kk_assert_file_exists "$hidden_file" "Hidden file exists"; then
    kk_test_pass "Hidden files are handled correctly"
else
    kk_test_fail "Hidden files not handled correctly"
fi

# Test with very long paths
kk_test_start "File assertions with long paths"
long_path="$TMPDIR"
for i in {1..10}; do
...
touch "$long_file"
if kk_assert_file_exists "$long_file" "Long path file"; then
    kk_test_pass "Long paths are handled correctly"
else
    kk_test_fail "Long paths not handled correctly"
fi

# Test with special characters in filenames
kk_test_start "File assertions with special characters"
special_file="$TMPDIR/file!@#$%^&*().txt"
touch "$special_file"
if kk_assert_file_exists "$special_file" "Special chars file"; then
    kk_test_pass "Special characters in filenames handled correctly"
else
    kk_test_fail "Special characters in filenames not handled correctly"
fi